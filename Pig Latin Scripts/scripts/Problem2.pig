...
business_filtered_CA = FILTER business_table BY (full_address matches '.* CA .*');

business_filtered_NOT_PA = FILTER business_filtered_CA BY NOT(full_address matches '.*Palo Alto, CA.*');
...
join_table = JOIN business_filtered_NOT_PA BY business_id, review_avg_table BY business_id;
...
STORE result INTO 'pig_output2' USING PigStorage('\t');
